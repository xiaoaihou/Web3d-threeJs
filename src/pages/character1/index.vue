      ...
      action: <span @click="handleAction(action)" v-for="(action, index) in actionsControls" :key="index">
        ...
        <Renderer ref="renderer" antialias :orbit-ctrl="{ enableDamping: true }" resize="window">
          ...
          <Scene :background="background">
            ...
            <GltfModel @onLoad="onLoad" src="src/characterModels/gltf/charman2/scene.gltf" :activeIndex="activeIndex" />
          </Scene>
        </Renderer>
  </div>
</template>
...
export default defineComponent({
  ...
  methods: {
    ...
    animate() {
      ...
      if (this.controls.staticPose) this.fadeToAction('staticPose', 0.5)
